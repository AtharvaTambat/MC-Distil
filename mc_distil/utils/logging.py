...
class Logger(object):
    def __init__(self, log_dir, seed, name="", create_model_dir=True, use_tf=False):
        """Create a summary writer logging to log_dir."""
        self.seed = int(seed)
        ...
        date_hour_minute = now_ist.strftime("%d-%m_%H%-M")
        if name == "":
            name = "seed-{:}-T-{:}.log".format(self.seed, date_hour_minute)
        if '.log' not in name:
            name += '.log'
