...
class NewResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=class_num, all_planes=[64, 128, 256, 512], adaptive_pool=False):
...
class ImageEncoder(nn.Module):
    def __init__(self):
        ...
        resnet = rnn_models.resnet18(pretrained=True)
        # Remove the fully connected layers to get the features before the classification layer
...
class TensorLayer(nn.Module):
    def __init__(self, num_tensors, tensor_size):
        super(TensorLayer, self).__init__()
        self.tensors = nn.ModuleList([nn.Linear(tensor_size, 1) for _ in range(num_tensors)])
        self.tensors= self.tensors.cuda()
...
class ResNet32(nn.Module):
    def __init__(self, num_classes, n):
        ...
        self.in_channels = 16
        self.conv = nn.Conv1d(1, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn = nn.BatchNorm1d(16)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self.make_layer(ResNetBlock, 16, 3, stride=1)
        self.layer2 = self.make_layer(ResNetBlock, 32, 3, stride=2)
        self.layer3 = self.make_layer(ResNetBlock, 64, 3, stride=2)
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Linear(64, 2 * n)
...
class ResNet32MetaNet(nn.Module):
    ...
    def __init__(self, block=BasicBlock, num_blocks=[5, 5, 5], use_sigmoid=False):
        ...
        self.in_planes = 16
        #self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv1 = nn.Conv2d(3, 16, kernel_size=2, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
...
class InstanceMetaNetLite(nn.Module):
    "A simple transformation for resnet features"
    def __init__(self, in_features=64, num_layers=1):
        super(InstanceMetaNetLite, self).__init__()
        self.num_layers = num_layers
        self.layers = []
        curr_dim = in_features
        for i in range(self.num_layers):
            self.layers.append(nn.Linear(curr_dim,curr_dim//2))
            curr_dim = curr_dim//2
        
        self.layers = nn.Sequential(*self.layers)
        self.final_layer = nn.Linear(curr_dim,2)
